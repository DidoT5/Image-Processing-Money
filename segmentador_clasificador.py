...
'''
def contadorDinero(img, descripcion=True, showSegmentacion = False):
    billetes, monedas = segmentador.analizaFoto(img,showSegmentacion)
    
    clasifBilletes = []
    for billete in billetes:
        res, maximum, media = clasificadorBillete.detectar(billete, False, False)
        if (res==13):
            if(descripcion):
